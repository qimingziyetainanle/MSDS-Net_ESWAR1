...
class SubspaceBranch(nn.Module):
    def __init__(self, in_dim: int, hidden_dim: int, dropout: float, use_separation: bool = True, cfg=None) -> None:
        super().__init__()
        self.cfg = cfg
    ...
    def forward(self, x: torch.Tensor) -> Dict[str, torch.Tensor]:
        ...
        if self.use_separation:
            ...
            mix = float(getattr(self.cfg, "a_residual_mix", 0.0))
            if mix > 0:
                A_res = x - L - E
...
def make_model_for_variant(variant: str, subspace_cols: Dict[str, List[str]], feature_cols: List[str], cfg: Config, device: torch.device) -> Tuple[MSDSNet, Dict[str, List[str]]]:
    ...
    if variant == "w/o Subspace":
        ...
        model = MSDSNet({"ALL": len(raw_cols)}, cfg.hidden_dim, cfg.graph_dim, cfg.dropout,
                        cfg.graph_score_weight, use_separation=True, use_graph=False, cfg=cfg,)
    else:
        cols = subspace_cols
        dims = {k: len(v) for k, v in cols.items()}
        if variant == "FULL":
            model = MSDSNet(dims, cfg.hidden_dim, cfg.graph_dim, cfg.dropout, cfg.graph_score_weight, cfg=cfg,
...
def run_single_seed_full(seed: int,
    ...
    threshold = choose_threshold(
        train_score=train_result["score"][:train_end],
        quantile=cfg.threshold_quantile,
        far_target=0.05,
        val_score=train_result["score"][train_end:],
...
def run_full_experiment(cfg):
    ...
    injected_feature_df, _, _ = add_engineered_features(
        injected_raw
    )
